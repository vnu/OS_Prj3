 ...
 *
 * DESCRIPTION	: This program acts as a fileserver to the fclient and as a disk client to the dserver.
 * 							   It gets the input from the fclient, processes it and pass the proper code through the socket to the dserver.
 *                            It gets the output from the dserver processes it and sends the appropriate output to the fclient
 *
 *
 *
 * EXECUTION		:	./fserver <PORT>  <IP> <Dserver Port>
 *
 * <PORT> Port Number to which fclient must connect to
 * <IP> is the dserver ip to connect to
 *  <Dserver PORT> is the dserver port to connect to
 *
 *
 * DATE				:	25 Apr 2011
 *
